...
class PdfConsulta extends FPDF
{
    ...
    public function Head($y)
    {
        ...
        $this->Cell(0, 20, utf8_decode('HOJA DE CONSULTA'), 0, 0, 'C');
    }
    ...
    public function Top($y, $dataconsulta)
    {
        ...
        $this->Cell(30, 5, $dataconsulta[0]->edad, 1, 0, 'L');
    ...
    public function Body($y, $dataconsulta, $dataservicio)
    {
        $x = 6;
        $this->setY($y);
        $this->setX($x);
        ...
        foreach ($dataconsulta as $ll => $consulta) {
            // print_r($consulta); die();
            $key = 0;
            $this->setY($y);
            $this->setX($x);
            $this->SetFont('Arial', 'B', 10);
            $this->Cell(177, 6, utf8_decode('Fecha Consulta: ') . $consulta->fecha, 0, 0, 'C');
            $this->SetFont('Arial', 'B', 10);
            $this->Cell(196, 6, utf8_decode('Peso: ') . $consulta->pesoMascota . ' KG', 0, 0, 'L');
            ...
            $total = 0;
            $y += 6;
            foreach ($dataservicio as $kk => $servicio) {
                $this->setY($y);
                $this->setX($x);
